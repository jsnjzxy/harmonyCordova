...
var nativeApiProvider = require('cordova/harmony/nativeapiprovider');
var utils = require('cordova/utils');
...
function harmonyExec (success, fail, service, action, args) {
    if (bridgeSecret < 0) {
    ...
    if (jsToNativeBridgeMode === undefined) {
        harmonyExec.setJsToNativeBridgeMode(jsToNativeModes.JS_OBJECT);
    }
    ...
    // If argsJson was received by Harmony as null, try again with the PROMPT bridge mode.
    // This happens in rare circumstances, such as when certain Unicode characters are passed over the bridge on a Galaxy S2.  See CB-2666.
    if (jsToNativeBridgeMode === jsToNativeModes.JS_OBJECT && msgs === '@Null arguments.') {
        harmonyExec.setJsToNativeBridgeMode(jsToNativeModes.PROMPT);
        harmonyExec(success, fail, service, action, args);
        harmonyExec.setJsToNativeBridgeMode(jsToNativeModes.JS_OBJECT);
    } else if (msgs) {
...
harmonyExec.init = function () {
    bridgeSecret = +prompt('', 'gap_init:' + nativeToJsBridgeMode);
    channel.onNativeReady.fire();
    console.log('Zxy_Cordova is fire');
};
...
harmonyExec.jsToNativeModes = jsToNativeModes;
harmonyExec.nativeToJsModes = nativeToJsModes;

harmonyExec.setJsToNativeBridgeMode = function (mode) {
...
harmonyExec.setNativeToJsBridgeMode = function (mode) {
    ...
    // Otherwise, it will be set by harmonyExec.init()
    if (bridgeSecret >= 0) {
...
// Processes a single message, as encoded by NativeToJsMessageQueue.
function processMessage (message) {
    var firstChar = message.charAt(0);
    if (firstChar === 'J') {
        // This is deprecated on the side. It doesn't work with CSP enabled.
...
function processMessages () {
    ...
    try {
        ...
        // The Harmony side can send a * message to indicate that it
...
// file: ../../cordova-js-src/harmony/nativeapiprovider.js
define("cordova/harmony/nativeapiprovider", function(require, exports, module) {

/**
 * Exports the ExposedJsApi object if available, otherwise exports the PromptBasedNativeApi.
 */

var nativeApi = this._cordovaNative || require('cordova/harmony/promptbasednativeapi');
var currentApi = nativeApi;

module.exports = {
    get: function () { return currentApi; },
    setPreferPrompt: function (value) {
        currentApi = value ? require('cordova/harmony/promptbasednativeapi') : nativeApi;
    },
    // Used only by tests.
    set: function (value) {
        currentApi = value;
...
// file: ../../cordova-js-src/harmony/promptbasednativeapi.js
define("cordova/harmony/promptbasednativeapi", function(require, exports, module) {

/**
 * Implements the API of ExposedJsApi, but uses prompt() to communicate.
...
module.exports = {
    ...
    bootstrap: function () {
        ...
        modulemapper.clobbers('cordova/plugin/harmony/app', 'navigator.app');

        var APP_PLUGIN_NAME = 'CoreHarmony';
        // Let native code know we are all done on the JS side.
        // Native code will then un-hide the WebView.
        channel.onCordovaReady.subscribe(function () {
            console.log('Zxy_Cordova is ready');
            exec(onMessageFromNative, null, 'CoreHarmony', 'messageChannel', []);
...
// file: ../../cordova-js-src/plugin/harmony/app.js
define("cordova/plugin/harmony/app", function(require, exports, module) {
